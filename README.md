...
A Python-based AI customer support agent built with the **Anthropic Claude API**, demonstrating a four-stage agentic workflow for SMB customer communication.
...
The Python layer reads this metadata to decide stage transitions and escalation, keeping the LLM focused only on generating good responses and accurate metadata.

### Escalation triggers

| Trigger | Source |
|---------|--------|
| Angry sentiment / complaint | Claude (`escalate: true`) |
| Medical question | Claude (`escalate: true`) |
| Pricing negotiation | Claude (`escalate: true`) |
| Out-of-scope question | Claude (`is_out_of_scope: true`) |
